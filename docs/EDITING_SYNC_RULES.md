...
[apps.mynewapp]
include = [
    "~/.config/mynewapp/**/*.conf",
]

exclude = [
    "~/.config/mynewapp/cache/**",
]
```
...
cat ~/projects/drifters/presets/zed.toml >> .drifters/sync-rules.toml
cat ~/projects/drifters/presets/nvim.toml >> .drifters/sync-rules.toml

# Commit
git add .drifters/sync-rules.toml
git commit -m "Add Zed and Neovim configs"
git push
```
...
drifters import file ~/Downloads/myapp.toml
```
